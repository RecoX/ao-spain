...
UserList(UserIndex).Stats.UserSkills(22) = val(US22) '[Efestos]
...
If UserList(UserIndex).Flags.Cabalgando = 1 Then
...
If UserList(UserIndex).Pos.Map = 0 Then
    If UCase$(UserList(UserIndex).Hogar) = "ARCADIA" Then
             UserList(UserIndex).Pos = Arcadia
...
'[Efestos]
If UserList(UserIndex).Flags.Cabalgando = 1 Then Call SendData(ToIndex, UserIndex, 0, "CABAL")
...
UserList(UserIndex).GuildInfo.Renuncias = 0
...
UserList(UserIndex).Flags.Paralizable = True
   ...
   If Not UserList(UserIndex).Flags.UserLogged Then
        ...
        Select Case Left$(rdata, 6)
            Case "PASSCL"
                ...
                Exit Sub
            Case "OLOGIN"
                If PassCliente <> "orophin" Then
                    Call LogHackAttemp("Intento entrar con un Cliente Alternativo:" & rdata)
                    Call CloseSocket(UserIndex)
                    Exit Sub
                End If
                rdata = Right$(rdata, Len(rdata) - 6)
...
'                If Not ValidInputNP(rdata) Then Exit Sub
                ...
                If VersionOK(Ver) Then
                     ...
                     If (UserList(UserIndex).Flags.ValCoDe = 0) Or (ValidarLoginMSG(UserList(UserIndex).Flags.ValCoDe) <> CInt(val(ReadField(38, rdata, 44)))) Then
                         Call LogHackAttemp("IP:" & frmMain.Socket2(UserIndex).PeerAddress & " intento crear un bot.")
                         Call CloseSocket(UserIndex)
                         Exit Sub
                     End If
...
Select Case UCase$(Left$(rdata, 1))
    ...
    Case "M" 'Moverse
        ...
        If Not UserList(UserIndex).Flags.Descansar And Not UserList(UserIndex).Flags.Meditando _
           And UserList(UserIndex).Flags.Paralizado = 0 And UserList(UserIndex).Flags.Paralizable = True Then
        ...
        ElseIf UserList(UserIndex).Flags.Meditando Then
          ...
          UserList(UserIndex).Char.loops = 0
          Call SendData(ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFX" & UserList(UserIndex).Char.CharIndex & "," & 0 & "," & 0)
        ElseIf UserList(UserIndex).Flags.Paralizable = False Then
          Call MoveUserChar(UserIndex, val(rdata))
        Else
        ...
        If UserList(UserIndex).Flags.Oculto = 1 Then
            ...
            If UCase$(UserList(UserIndex).Clase) <> "LADRON" And UCase$(UserList(UserIndex).Clase) <> "GUERRERO" And UCase$(UserList(UserIndex).Clase) <> "CAZADOR" Then
                'si es ladron, guerrero o cazador no se hace visible cuando camina
...
Select Case UCase$(Left$(rdata, 6))
    Case "/CLAN "
        ...
        If rdata <> "" And UserList(UserIndex).GuildInfo.GuildName <> "" Then
            Call SendData(ToGuildMembers, UserIndex, 0, "||" & "Mensaje de Clan: " & rdata & FONTTYPE_GUILD)
        End If
        Exit Sub
End Select
...
Select Case UCase$(rdata)
    ...
    Case "/ONLINE"
        ...
        Exit Sub
    Case "/SALIR"
        If UserList(UserIndex).Flags.Ceguera = 1 Or UserList(UserIndex).Flags.Paralizado = 1 Then
    ...
    Case "/ONLINECLAN"
        Dim clan As String
        clan = UserList(UserIndex).GuildInfo.GuildName
        For LoopC = 1 To LastUser
            If (UserList(LoopC).Name <> "") And UserList(LoopC).GuildInfo.GuildName = clan Then
                tStr = tStr & UserList(LoopC).Name & ", "
            End If
        Next LoopC
        If Len(tStr) > 0 Then
            ...
            Call SendData(ToIndex, UserIndex, 0, "||" & tStr & FONTTYPE_INFO)
        Else
            Call SendData(ToIndex, UserIndex, 0, "||No hay PJs de tu clan Online" & FONTTYPE_INFO)
        End If
        Exit Sub
    Case "/MUERTES"
        Call SendData(ToIndex, UserIndex, 0, "Stats:" & FONTTYPE_INFO)
        Call SendData(ToIndex, UserIndex, 0, "||Criminales matados: " & UserList(UserIndex).Faccion.CriminalesMatados & FONTTYPE_INFO)
        Call SendData(ToIndex, UserIndex, 0, "||Ciudadanos matados matados: " & UserList(UserIndex).Faccion.CiudadanosMatados & FONTTYPE_INFO)
        Call SendData(ToIndex, UserIndex, 0, "||Npcs matados: " & UserList(UserIndex).Stats.NPCsMuertos & FONTTYPE_INFO)
        Exit Sub
    Case "/RESUCITAR"
       'Se asegura que el target es un npc
       If UserList(UserIndex).Flags.TargetNpc = 0 Then
    ...
    Case "/AYUDA"
       ...
       Exit Sub
    Case "/SALIRCLAN"
        If UserList(UserIndex).GuildInfo.EsGuildLeader = 1 Then Exit Sub
        If UserList(UserIndex).GuildInfo.GuildName = "" Then Exit Sub
        If UserList(UserIndex).GuildInfo.Renuncias > 3 Then
            Call SendData(ToIndex, UserIndex, 0, "||No puedes retirarte del clan, superaste el maximo de 3 renuncias." & FONTTYPE_GUILD)
            Exit Sub
        End If
        Dim oGuild As cGuild
        Set oGuild = FetchGuild(UserList(UserIndex).GuildInfo.GuildName)
        If oGuild Is Nothing Then Exit Sub
        Call SendData(ToIndex, UserIndex, 0, "||Te has ido del clan." & FONTTYPE_GUILD)
        Call AddtoVar(UserList(UserIndex).GuildInfo.Renuncias, 1, 1000)
        UserList(UserIndex).GuildInfo.GuildPoints = 0
        UserList(UserIndex).GuildInfo.GuildName = ""
        Call SendData(ToGuildMembers, UserIndex, 0, "||" & UserList(UserIndex).Name & " se ha ido del clan." & FONTTYPE_GUILD)
     ...
     Case "/EST"
        ...
        Exit Sub
    Case "/ESTADISTICAS"
        Call SendData(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & "En total has matado " & UserList(UserIndex).Faccion.CiudadanosMatados & " ciudadanos" & "y " & UserList(UserIndex).Faccion.CriminalesMatados & " criminales" & "°" & Npclist(UserList(UserIndex).Flags.TargetNpc).Char.CharIndex & FONTTYPE_INFO)
        Exit Sub
    Case "ATRI"
...
Select Case UCase$(Left$(rdata, 2))
    Case "TI" 'Tirar item
            If UserList(UserIndex).Flags.Navegando = 1 Or UserList(UserIndex).Flags.Cabalgando = 1 Or _
    ...
    Case "UK"
        ...
        Select Case val(rdata)
            ...
            Case Ocultarse
                ...
                '[Efestos]
                If UserList(UserIndex).Flags.Cabalgando = 1 Then
                      Call SendData(ToIndex, UserIndex, 0, "||No podes ocultarte si estas cabalgando." & FONTTYPE_INFO)
                      Exit Sub
                End If
...
Select Case UCase$(Left$(rdata, 3))
    ...
    Case "CNC" ' Construye carpinteria
        ...
        rdata = Right$(rdata, Len(rdata) - 3)
        Numeros = UCase$(Left$(rdata, 1))
        Select Case Numeros
            Case 1
                Cantidad = UCase$(Right$(rdata, 1))
                rdata = Left$(rdata, Len(rdata) - 1)
                rdata = Right$(rdata, Len(rdata) - 1)
            Case 2
                Cantidad = UCase$(Right$(rdata, 2))
                rdata = Left$(rdata, Len(rdata) - 2)
                rdata = Right$(rdata, Len(rdata) - 1)
            Case 3
                Cantidad = UCase$(Right$(rdata, 3))
                rdata = Left$(rdata, Len(rdata) - 3)
                rdata = Right$(rdata, Len(rdata) - 1)
            Case 4
                Cantidad = UCase$(Right$(rdata, 4))
                rdata = Left$(rdata, Len(rdata) - 4)
                rdata = Right$(rdata, Len(rdata) - 1)
        End Select
        X = CInt(rdata)
...
If UCase$(Left$(rdata, 7)) = "/TOGMS " Then
    ...
    If rdata <> "" Then
        Call SendData(ToAdmins, 0, 0, "||" & rdata & FONTTYPE_TALK & ENDC)
    End If
    Exit Sub
End If
...
End If
'crear objetos
If UCase(Left(rdata, 6)) = "/CREAR" Then
rdata = Right(rdata, Len(rdata) - 7)
Call LogGM(UserList(UserIndex).Name, "/CREAR: " & rdata, False)
Dim MiObj As Obj

MiObj.Amount = ReadField(2, rdata, 32)
MiObj.ObjIndex = ReadField(1, rdata, 32)
...
If UCase$(Left$(rdata, 5)) = "/AI1 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   ArmaduraPaladinImperial = val(rdata)
   Exit Sub
End If

If UCase$(Left$(rdata, 5)) = "/AI2 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   ArmaduraClerigoImperial = val(rdata)
   Exit Sub
End If

If UCase$(Left$(rdata, 5)) = "/AI3 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   ArmaduraEnanoImperial = val(rdata)
   Exit Sub
End If

If UCase$(Left$(rdata, 5)) = "/AI4 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   TunicaMagoImperial = val(rdata)
   Exit Sub
End If
If UCase$(Left$(rdata, 5)) = "/AI5 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   TunicaMagoImperialEnanos = val(rdata)
   Exit Sub
End If
If UCase$(Left$(rdata, 5)) = "/AI6 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   TunicaMagoImperialHobbits = val(rdata)
   Exit Sub
End If
If UCase$(Left$(rdata, 5)) = "/AI7 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   ArmaduraImperialMujer = val(rdata)
   Exit Sub
End If
If UCase$(Left$(rdata, 5)) = "/AI8 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   TunicaMagoImperialMujer = val(rdata)
   Exit Sub
End If
If UCase$(Left$(rdata, 5)) = "/AC1 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   ArmaduraPaladinCaos = val(rdata)
   Exit Sub
End If

If UCase$(Left$(rdata, 5)) = "/AC2 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   ArmaduraEnanoCaos = val(rdata)
   Exit Sub
End If

If UCase$(Left$(rdata, 5)) = "/AC3 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   TunicaMagoCaos = val(rdata)
   Exit Sub
End If

If UCase$(Left$(rdata, 5)) = "/AC4 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   TunicaMagoCaosEnanos = val(rdata)
   Exit Sub
End If
If UCase$(Left$(rdata, 5)) = "/AC5 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   TunicaMagoCaosHobbits = val(rdata)
   Exit Sub
End If
If UCase$(Left$(rdata, 5)) = "/AC6 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   ArmaduraCaosMujer = val(rdata)
   Exit Sub
End If
If UCase$(Left$(rdata, 5)) = "/AC7 " Then '[Neptuno]
   rdata = Right$(rdata, Len(rdata) - 5)
   TunicaMagoCaosMujer = val(rdata)
   Exit Sub
End If
...
If UCase$(rdata) = "/APAGAR" Then
    If UCase$(UserList(UserIndex).Name) <> "Neptuno" Then
