...
Select Case UCase$(Clase)
    ...
    Case "ARQUERO" 'Neptuno
        ModificadorEvasion = 0.9
    Case "GLADIADOR" 'Neptuno
...
Select Case UCase$(Clase)
    ...
    Case "GLADIADOR" 'Neptuno
        ModificadorPoderAtaqueArmas = 1.1
    Case "GUERRERO"
...
Select Case UCase$(Clase)
    Case "ARQUERO" 'Neptuno
        ModificadorPoderAtaqueProyectiles = 1.2
    Case "GLADIADOR" 'Neptuno
    ...
    Case "DRUIDA"
        ModificadorPoderAtaqueProyectiles = 0.75
    Case "ALDEANO" 'Neptuno
        ModificadorPoderAtaqueProyectiles = 0.6
    Case "PESCADOR"
...
'si esta en tierra, no añade el daño
...
'[Efestos] Si el barco no esta en agua, no se le suma el daño adicional
If UserList(UserIndex).Flags.Navegando = 1 And HayAgua(UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y) = True Then _
...
If UserList(UserIndex).Flags.Cabalgando = 1 Then _
        daño = daño + RandomNumber(ObjData(UserList(UserIndex).Invent.CaballoObjIndex).MinHIT, ObjData(UserList(UserIndex).Invent.CaballoObjIndex).MaxHIT)
...
Dim Obj As ObjData
Dim defCaballo As Integer


daño = RandomNumber(Npclist(NpcIndex).Stats.MinHIT, Npclist(NpcIndex).Stats.MaxHIT)
antdaño = daño
'[Efestos] Si el barco no esta en agua, no se le suma la defensa adicional
If UserList(UserIndex).Flags.Navegando = 1 And HayAgua(UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y) = True Then
...
If UserList(UserIndex).Flags.Cabalgando = 1 Then
    Obj = ObjData(UserList(UserIndex).Invent.CaballoObjIndex)
    defCaballo = RandomNumber(Obj.MinDef, Obj.MaxDef)
End If
...
Select Case Lugar
  Case bCabeza
        ...
        If UserList(UserIndex).Invent.CascoEqpObjIndex > 0 Then
           ...
           absorbido = absorbido + defbarco + defCaballo
           daño = daño - absorbido
...
antdaño = daño
'[Efestos]
If UserList(AtacanteIndex).Flags.Navegando = 1 And HayAgua(UserList(AtacanteIndex).Pos.Map, UserList(AtacanteIndex).Pos.X, UserList(AtacanteIndex).Pos.Y) = True Then
     Obj = ObjData(UserList(AtacanteIndex).Invent.BarcoObjIndex)
     daño = daño + RandomNumber(Obj.MinHIT, Obj.MaxHIT)
End If

If UserList(VictimaIndex).Flags.Navegando = 1 And HayAgua(UserList(VictimaIndex).Pos.Map, UserList(VictimaIndex).Pos.X, UserList(VictimaIndex).Pos.Y) = True Then
     Obj = ObjData(UserList(VictimaIndex).Invent.BarcoObjIndex)
     defbarco = RandomNumber(Obj.MinDef, Obj.MaxDef)
End If

If UserList(AtacanteIndex).Flags.Cabalgando = 1 Then
     Obj = ObjData(UserList(AtacanteIndex).Invent.CaballoObjIndex)
     daño = daño + RandomNumber(Obj.MinHIT, Obj.MaxHIT)
End If

If UserList(VictimaIndex).Flags.Cabalgando = 1 Then
     Obj = ObjData(UserList(VictimaIndex).Invent.CaballoObjIndex)
     defCaballo = RandomNumber(Obj.MinDef, Obj.MaxDef)
End If
...
End If
'[Efestos]
If UserList(AttackerIndex).GuildInfo.GuildName = UserList(VictimIndex).GuildInfo.GuildName And UserList(AttackerIndex).GuildInfo.GuildName <> "" Then
    Call SendData(ToIndex, AttackerIndex, 0, "||No puedes atacar a los miembros de tu clan." & FONTTYPE_WARNING)
    PuedeAtacar = False
    Exit Function
End If
'[Efestos]
