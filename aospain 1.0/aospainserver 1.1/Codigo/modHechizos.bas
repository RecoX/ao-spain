...
ElseIf Hechizos(Spell).SubeHP = 2 Then
    ...
    Call SendData(ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFX" & UserList(UserIndex).Char.CharIndex & "," & Hechizos(Spell).FXgrh & "," & Hechizos(Spell).loops)
    '[Efestos]
    resist = ((daño * (UserList(UserIndex).Stats.UserSkills(Resistencia)) / 4)) / 100
    daño = daño - resist
    '[Efestos]
    ...
    Call SendData(ToIndex, UserIndex, 0, "||" & Npclist(NpcIndex).Name & " te ha quitado " & daño & " puntos de vida." & FONTTYPE_FIGHT)
    '[Efestos]
    Call SendData(ToIndex, UserIndex, 0, "||" & "La Resistencia magica te ha prevenido " & resist & " puntos de daño." & FONTTYPE_FIGHT)
    '[Efestos]
    'Muere
...
ElseIf Hechizos(H).SubeHP = 2 Then
    ...
    Call InfoHechizo(UserIndex)
'[Efestos]
    resist = val(((daño * (UserList(UserIndex).Stats.UserSkills(Resistencia) / 4)) / 100))
    daño = daño - resist
'[Efestos]
...
'[Efestos]
    Call SendData(ToIndex, tempChr, 0, "||" & "La resistencia magica te ha prevenido " & resist & " puntos de daño." & FONTTYPE_FIGHT)
'[Efestos]
    'Muere
