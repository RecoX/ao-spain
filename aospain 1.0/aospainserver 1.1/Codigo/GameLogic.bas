...
Public Function EsCasado(ByVal UserIndex As Integer) As Boolean
EsCasado = UserList(UserIndex).Stats.ELV <> ""
End Function
...
If InMapBounds(Map, X, Y) Then
    ...
    If FoundChar = 1 Then '  ¿Encontro un Usuario?
       ...
       If UserList(TempCharIndex).Flags.AdminInvisible = 0 Then
            ...
            End If
            '[Efestos]
            If Criminal(TempCharIndex) And UserList(TempCharIndex).Flags.Privilegios <= 1 Then
                Stat = Stat & " <CRIMINAL> ~255~0~0~1~0"
            ElseIf UserList(TempCharIndex).Flags.Privilegios > 1 Then
                Stat = Stat & " <GM> ~0~200~0~1~0"    '[Efestos]
            Else
